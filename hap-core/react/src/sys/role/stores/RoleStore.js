...
class RoleStore {
  ...
  checkUserName(name) {
    return axios.get(`/v1/public/role/name?name=${name}`);
  }
  ...
  getWholePermission(level, page, filters = {
    ...
    return Observable.fromPromise(axios.get(`/v1/permissions?level=${level}&page=${page.current}&size=${page.pageSize}&code=${filters.code}&params=${filters.params}`));
  }

  loadRoleLabel = (level) => {
    axios.get(`/v1/labels?type=role&level=${level}`).then((data) => {
      this.setLabel(data);
    }).catch((error) => {
  ...
  loadRole(level, { current = 1, pageSize },
    ...
    return axios.post(
      `/v1/roles/search?page=${current}&size=${pageSize}&sort=${sorter.join(',')}`,
      JSON.stringify(body),
    );
  }
  ...
  queryRole = (state) => {
    if (state.code === '') {
      axios.get(`/v1/roles?param=${state.input}&page=1&size=10`).then((data) => {
        this.setRoles(data.list);
      });
    } else {
      axios.get(`/v1/roles?${state.code}=${state.input}&page=1&size=10`).then((data) => {
        this.setRoles(data.list);
      });
    }
  };

  getSelectedRolePermissions = ids => axios.post('/v1/permissions', ids);

  createRole = role => axios.post('/v1/roles', role);

  getRoleById = id => axios.get(`/v1/roles/${id}`);

  updateRoleById = (id, role) => axios.put(`/v1/roles/${id}`, JSON.stringify(role));

  deleteRoleById = id => axios.delete(`/v1/roles/${id}`);

  editRoleByid = (id, data) => axios.put(`/v1/roles/${id}`, data);

  enableRole = id => axios.put(`/v1/roles/${id}/enable`);

  disableRole = id => axios.put(`/v1/roles/${id}/disable`);
  ...
  queryDatas([page, size], value) {
    ...
    this.setIsLoading(true);
    return axios.get(`/v1/roles?page=${page}&size=${size}${url}`).then((data) => {
      if (data) {
        this.setRoles(data.list);
