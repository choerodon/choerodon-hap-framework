...
import { axiosPro as axios, ContentPro as Content, Header, Page, stores } from '@choerodon/boot';
...
export default class MenuSetting extends Component {
  ...
  getRowKey = record => `${record.parentCode} - ${record.code}`;
  ...
  pushMenuItem = (menu, arr) => {
    arr.push({ ...menu, subMenus: null });
    if (menu.subMenus && menu.subMenus.length) {
      menu.subMenus.forEach(m => this.pushMenuItem(m, arr));
    }
  }
  ...
  handleStatus = () => {
    const origins = [];
    this.copy.forEach(m => this.pushMenuItem(m, origins));
