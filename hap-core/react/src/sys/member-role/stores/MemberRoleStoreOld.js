...
/**
 * 当要改写 src/app/containers/global/member-role/MemberRoleType.js中的内容时可以逐步把用到的东西移到store里
 */
...
class MemberRoleStore {
  ...
  loadCurrentMenuType(data, userId) {
    this.data = data;
    this.userId = userId;
    const { type = 'site', id = 0, name } = this.data;
    // change iam2null sign
    let apiGetway = `/v1/${type}s/${id}`;
    let codePrefix;
    switch (type) {
      case 'organization':
  ...
  // change iam2null sign
  loadUsers = (queryObj = { userName: '' }) => axios.get(`/v1/all/users?${querystring.stringify(queryObj)}`);

  // change iam2null sign
  loadClients = (queryObj = { sort: 'id' }) => axios.get(`/v1/all/clients?${querystring.stringify(queryObj)}`);
}
